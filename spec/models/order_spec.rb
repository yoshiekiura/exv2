describe Order, 'validations', type: :model do
  ...
  context 'limit order' do
    it 'should make sure price is present' do
      order = Order.new(market_id: 'btccny', price: nil, ord_type: 'limit')
      expect(order).not_to be_valid
      expect(order.errors[:price]).to eq ['is not a number']
    end

    it 'should make sure price is greater than zero' do
      order = Order.new(market_id: 'btccny', price: '0.0'.to_d, ord_type: 'limit')
      expect(order).not_to be_valid
  ...
  context 'market order' do
    it 'should make sure price is not present' do
      order = Order.new(market_id: 'btccny', price: '0.0'.to_d, ord_type: 'market')
      expect(order).not_to be_valid
...
describe Order, '#fix_number_precision', type: :model do
  let(:order_bid) { create(:order_bid, market_id: 'btccny', price: '12.326'.to_d, volume: '123.123456789') }
  let(:order_ask) { create(:order_ask, market_id: 'btccny', price: '12.326'.to_d, volume: '123.123456789') }
